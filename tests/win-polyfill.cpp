...
ImplementGetModuleHandleExW(DWORD dwFlags, LPCWSTR lpModuleName, HMODULE *phModule)
{
    ...
    if (dwFlags & GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS)
    {
        *phModule = NULL;
        ...
        const LPFN_RtlPcToFileHeader pfnRtlPcToFileHeader =
            reinterpret_cast<LPFN_RtlPcToFileHeader>(
                GetProcAddress(hmodkernel32, "RtlPcToFileHeader"));
        ...
        else
        {
            ...
            if (VirtualQuery((PVOID)lpModuleName, &mbi, sizeof(mbi)) >=
                offsetof(MEMORY_BASIC_INFORMATION, AllocationProtect))
            {
...
#define LDR_ADDREF_DLL_PIN 0x00000001
        ...
        const LPFN_LdrAddRefDll pfnLdrAddRefDll = reinterpret_cast<LPFN_LdrAddRefDll>(
            GetProcAddress(hmodntdll, "LdrAddRefDll"));

        if (pfnLdrAddRefDll)
        {
            // update dll reference
            if (!NT_SUCCESS(pfnLdrAddRefDll(
                    (dwFlags & GET_MODULE_HANDLE_EX_FLAG_PIN) ? LDR_ADDREF_DLL_PIN : 0,
                    *phModule)))
            {
        ...
        {
            WCHAR *filename;
            if ((filename = reinterpret_cast<WCHAR *>(VirtualAlloc(
                     NULL, MAX_PATH * sizeof(WCHAR), MEM_COMMIT, PAGE_READWRITE))) !=
                NULL)
            {
...
//
extern "C" BOOL WINAPI ImplementSetFilePointerEx(
    HANDLE hFile,
    ...
    DWORD dwMoveMethod)
{
    DWORD ret = SetFilePointer(
        hFile, liDistanceToMove.LowPart, &liDistanceToMove.HighPart, dwMoveMethod);
